...
namespace RayTracer
{
    ...
    public class Plane : SceneEntity
    {
        ...
        public RayHit Intersect(Ray ray)
        {
            ...
            // First, calculate the dot product of the plane normal vector and ray direction vector.
            double rayNormDot = ray.Direction.Dot(this.normal);

            if (Math.Abs(rayNormDot) < SceneEntity.EPSILON)
            {
                return null;
            }

            double tParameter = this.normal.Dot(this.center - ray.Origin) / rayNormDot;
            
            if (tParameter < SceneEntity.EPSILON)
            {
                return null;
            }

            Vector3 intersectPosition = ray.Origin + ray.Direction * tParameter;
            return new RayHit(intersectPosition, this.normal, ray.Direction, this.material);
        }
